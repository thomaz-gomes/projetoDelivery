...
export function buildDanfeText(data, opts = {}) {
  ...
  rawItems.forEach((it) => {
    ...
    // Placeholder R$0,01 não se aplica a combo (preço do combo é cheio).
    const isPlaceholder = !isCombo && hasOptions && basePrice > 0 && basePrice <= 0.10
    ...
    for (const opt of opts) {
      if (opt.kind === 'combo_slot') {
        // Slot do combo: renderiza como sub-linha indentada, sem número e
        // sem preço (o valor já está no preço do combo na linha anterior).
        const label = opt.slotName ? `${opt.slotName}: ${opt.name || ''}` : (opt.name || '')
        expandedItems.push({ name: label, qty: 1, unit: 0, isSlot: true })
        continue
      }
  ...
  expandedItems.forEach((it) => {
    if (it.isSlot) {
      // Slot do combo: sub-linha indentada sem número, sem código, sem preço.
      // O preço foi contabilizado na linha do combo-pai imediatamente acima.
      const label = String(it.name || '').slice(0, W - 4)
      lines.push(`    ${label}`)
      return
    }
    itemNum += 1
