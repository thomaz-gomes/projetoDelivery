...
nfeRouter.get('/danfe/:orderId', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN', 'ATTENDANT'), async (req, res) => {
  try {
    ...
    if (!protocol) return res.status(404).send('NF-e não emitida para este pedido')

    const rawXml = protocol.rawXml || ''

    // Extract fields from SEFAZ response XML
    ...
    // Company / store info from fiscal config + store record
    ...
    // QR Code: generate from consultation URL + chave
    ...
    // Tributos (simplified estimate: ~7% federal + ~12% estadual + ~5% municipal = 24%)
