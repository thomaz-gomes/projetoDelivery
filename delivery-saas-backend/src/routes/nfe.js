...
import { sendNfeXmlEmail, sendNfePdfEmail } from '../services/email.js'
import { generateDanfeHtml } from '../services/nfeDanfeHtml.js'
import { renderHtmlToPdf } from '../services/pdfFromHtml.js'
...
// ══════════════════════════════════════════════════════════════════════════════
// DANFE NFC-e — HTML para impressão
// GET /nfe/danfe/:orderId — HTML imprimível (forPrint=true: botão + auto-print)
// ══════════════════════════════════════════════════════════════════════════════
nfeRouter.get('/danfe/:orderId', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN', 'ATTENDANT'), async (req, res) => {
  try {
    const { orderId } = req.params
    const companyId = req.user.companyId
    const html = await generateDanfeHtml(orderId, companyId, { forPrint: true })
    res.setHeader('Content-Type', 'text/html; charset=utf-8')
    return res.send(html)
  } catch (err) {
    if (err?.status === 404) return res.status(404).send(err.message)
    console.error('danfe error:', err)
    return res.status(500).send('Erro ao gerar DANFE: ' + (err?.message || String(err)))
  }
})

// GET /nfe/danfe-pdf/:orderId — DANFE renderizado como PDF para download
// ══════════════════════════════════════════════════════════════════════════════
nfeRouter.get('/danfe-pdf/:orderId', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN', 'ATTENDANT'), async (req, res) => {
  try {
    const { orderId } = req.params
    const companyId = req.user.companyId
    const html = await generateDanfeHtml(orderId, companyId, { forPrint: false })
    const pdf = await renderHtmlToPdf(html)
    const protocol = await prisma.nfeProtocol.findFirst({ where: { orderId }, orderBy: { createdAt: 'desc' } })
    const filename = `nfe-${protocol?.nProt || orderId}.pdf`
    res.setHeader('Content-Type', 'application/pdf')
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`)
    return res.send(pdf)
  } catch (err) {
    if (err?.status === 404) return res.status(404).json({ error: err.message })
    console.error('danfe-pdf error:', err)
    return res.status(500).json({ error: 'Erro ao gerar PDF: ' + (err?.message || String(err)) })
  }
})


// ══════════════════════════════════════════════════════════════════════════════
// DANFE NFC-e preview (modo debug) — gera DANFE a partir do XML sem protocolo
// POST /nfe/danfe-preview
// ══════════════════════════════════════════════════════════════════════════════
nfeRouter.post('/danfe-preview', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN', 'ATTENDANT'), async (req, res) => {
  try {
    const { xml: rawXml, orderId } = req.body
    if (!rawXml || !orderId) return res.status(400).send('xml e orderId são obrigatórios')
    ...
    if (!order || order.companyId !== companyId) return res.status(404).send('Pedido não encontrado')

    const extract = (tag, xml) => { const m = xml.match(new RegExp(`<${tag}[^>]*>([^<]+)</${tag}>`)); return m ? m[1].trim() : '' }

    // chNFe: in signed XML it lives in the Id attribute of infNFe (NFe + 44 digits)
    let chNFe = extract('chNFe', rawXml)
    if (!chNFe) {
      const idMatch = rawXml.match(/Id=["']NFe(\d{44})["']/)
      chNFe = idMatch ? idMatch[1] : ''
    }

    const dhEmi = extract('dhEmi', rawXml) || new Date().toISOString()
...
<title>DANFE NFC-e (DEBUG) #${nNF}</title>
<style>
  @page { margin: 5mm; }
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .debug-banner { background: #ff6b00; color: #fff; text-align: center; font-weight: bold; padding: 4px; margin: 4px 0; font-size: 11px; border-radius: 3px; }
...
<div class="debug-banner">⚠️ MODO DEBUG — SEM VALOR FISCAL ⚠️<br><span style="font-size:9px;font-weight:normal">NF-e NÃO foi transmitida ao SEFAZ</span></div>
...
<div class="section">
  <div class="total-row"><span>QTD. TOTAL DE ITENS</span><span>${totalItens}</span></div>
  <div class="total-row"><span>VALOR TOTAL R$</span><span>${vNFDisplay}</span></div>
</div>
...
<div class="chave">${chaveFormatted || '(chave não disponível)'}</div>

<div class="qr-section">
  ${qrDataUrl ? `<img class="qr-img" src="${qrDataUrl}" alt="QR Code NFC-e">` : ''}
  <div class="qr-info">
    <div class="bold" style="margin-bottom:2px;color:#ff6b00">SEM PROTOCOLO (DEBUG)</div>
...
<script>window.addEventListener('load', () => setTimeout(() => window.print(), 400))<\/script>
    ...
    console.error('danfe-preview error:', err)
    return res.status(500).send('Erro ao gerar prévia DANFE: ' + (err?.message || String(err)))
  }
})
...
// POST /nfe/enviar-email — send NF-e DANFE PDF as email attachment.
// Previously sent the raw XML; the operator workflow shifted to PDF since
// the customer wants a readable copy of the receipt, not the SEFAZ XML.
// The route accepts the same body shape (nfeProtocolId or orderId) so the
// frontend call site doesn't need to change beyond the button label.
nfeRouter.post('/enviar-email', authMiddleware, requireRole('ADMIN', 'SUPER_ADMIN'), requireFiscalModule, async (req, res) => {
  try {
    ...
    if (!protocol) return res.status(404).json({ error: 'Protocolo não encontrado' })
    const targetOrderId = protocol.orderId || orderId
    if (!targetOrderId) return res.status(400).json({ error: 'Pedido não vinculado ao protocolo' })

    let orderDisplay = null
    const ord = await prisma.order.findUnique({ where: { id: targetOrderId }, select: { displaySimple: true } })
    orderDisplay = ord?.displaySimple || null

    // Render the same DANFE HTML the print view uses, then puppeteer it to
    // a PDF attached to the e-mail.
    const html = await generateDanfeHtml(targetOrderId, companyId, { forPrint: false })
    const pdfBuffer = await renderHtmlToPdf(html)

    await sendNfePdfEmail(email, { pdfBuffer, nProt: protocol.nProt, orderDisplay })
